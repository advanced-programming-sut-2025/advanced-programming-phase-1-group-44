...
public class SignupMenuController extends MenuController{
    ...
    public void register(miniPlayer miniMan) {
        App.addPlayer(new Player(miniMan.username, miniMan.password, miniMan.nickname, miniMan.email, miniMan.gender));
    }
...
//        if (username.equals("")) return new Result(Map.of("message", "this is message", "isValid", false));
        ...
        Player newPlayer = new Player(username, password, nickName, email, gender);
        App.addPlayer(newPlayer);

        Main.getNetworkClient().register(newPlayer);
    ...
    public Result login(String username, String password, String stayLoggedIn) {
        ...
        Main.getNetworkClient().login(user);
